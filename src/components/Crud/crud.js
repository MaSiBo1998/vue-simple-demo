...
function CRUD(options) {
  ...
  const methods = {
    ...
    addSuccessNotify(msg) {
      crud.notify(msg, CRUD.NOTIFICATION_TYPE.SUCCESS)
    },
    editSuccessNotify() {
     ...
     * 通用导出
     */
    doExport(url, params, exportType, dayOrMonth) {
      crud.downloadLoading = true
      download(url, params, exportType, dayOrMonth).then(result => {
        console.log(result.data)
        const name = decodeURIComponent(result.headers['content-disposition'].split('"')[1])
    ...
    getQueryParams() {
      ...
      if (crud.query.timeRanges == undefined) {
        // crud.query.startTime = undefined
        // crud.query.endTime = undefined
      } else {
        // crud.query.startTime = crud.query.timeRanges[0]
        // crud.query.endTime = crud.query.timeRanges[1]
      }

      Object.keys(crud.query).length !== 0 && Object.keys(crud.query).forEach(item => {
        // if (Array.isArray(crud.query[item])) {
        //   crud.query[`${item}Start`] = crud.query[item][0]
        //   crud.query[`${item}End`] = crud.query[item][1]
        //   delete crud.query[item]
        // console.log('fdsfa',crud.query[item].length)
        // }
      ...
      return {
        ...
        // sort: crud.sort,
        // startTime: crud.query.timeRanges ? crud.query.timeRanges[0] : '',
        // endTime: crud.query.timeRanges ? crud.query.timeRanges[1] : '',
        ...crud.query,
        ...crud.params
      }
    },
